...
const {sendImageToMastodon,sendReplyImageToMastodon} = require("./mastodon.js");
...
const exclamation = require("./bits/exclamations.js");
...
(function(){
  let myConsole = new console.Console(fs.createWriteStream(`./logs/output${new Date().getTime()}.txt`));
...
const botScript = async () => {
  ...
  const statusText = `${indefinite(ajective)} new palette, just for you: #color #ColorPalette`;
...
const replyBotScript = async (acct, id) => {
  ...
  const drawReplyPaletteFilePath = __dirname + "/paletts/" + drawReplyPaletteFileName;
  ...
  return sendReplyImageToMastodon(drawReplyPaletteFilePath, imageDescription, statusText, replyId)
    .then(() => {
      console.log(`🤞 Hopefully, we've sent a canvas with the following colors: ${palette.hexCola}, ${palette.hexColb} ${palette.hexColc}, ${palette.hexCold}, and ${palette.hexCole} at ${new Date().toTimeString()}`
      );
      delete require.cache[require.resolve("./bits/ajectives-pos.js")];
      delete require.cache[require.resolve("./bits/random-from-array.js")];
      delete require.cache[require.resolve("./srcpg.js")];
...
console.log("📡 listening hard for super cool comments!");
stream.on("message", (response) => {
  if (response.event === "notification" && response.data.type === "mention") {
    ...
    fs.writeFileSync(`logs/messageData${new Date().getTime()}.json`,JSON.stringify(response.data.status, null, 2)
    );
    sleep(1000).then(() => {console.log(`"🎉 someone tagged us! ${randomFromArray(exclamation)}! 🎊"`);});
    sleep(1000).then(() => {replyBotScript(acct, id);});
    sleep(5000).then(() => {console.log(`"✨ ${randomFromArray(exclamation)}! ✨"`);});
  }
});
...
delete require.cache[require.resolve("./bits/exclamations.js")];
delete require.cache[require.resolve("./bits/ajectives-pos.js")];
delete require.cache[require.resolve("./bits/random-from-array.js")];
...
cron.schedule('*/30 * * * *', () => {
  console.log(`"🕰 at the tone the time will be: ${new Date().toTimeString()}...BEEEEEEEEP! ${randomFromArray(exclamation)}! 🔔"`);
  });
